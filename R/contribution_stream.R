...
#' @param event_subtypes2 list of formulas to be used for coding `event_subtype2`, as for [dplyr::case_when]. Can refer to any columns in the `contributions` table. See **Note** below
#' @note
#' * This value for __event_subtypes__:
#'   ```
#'   list(
...
#' * This value for __event_subtypes2__, which is also the default:
#'   ```
#'   list(
#'      1:.N == 1 ~ "New",
...
contribution_stream <- function(type = "stream",
  ...
  . <- group_customer_no <- cont_dt <- cont_amt <- campaign_no <- ref_no <- cust_memb_no <- contribution_amt <- contribution_max <-
    contribution_timestamp_min <- contribution_timestamp_max <- contribution_timestamp_last <- event_subtype <- event_subtype2 <- NULL

  event_subtypes <- substitute(event_subtypes)
  event_subtypes2 <- substitute(event_subtypes2)
  ...
  contribution_stream <- contributions[cont_amt > 0] %>%
    .[, `:=`(
      ...
      contribution_timestamp_max = cont_dt
    ), by = "group_customer_no"] %>%
    merge(
      .[, .(
        cont_dt = unique(cont_dt),
        contribution_timestamp_last = lag(unique(cont_dt))
      ), by = "group_customer_no"],
  ...
  # set event_subtype2
  contribution_stream[, event_subtype2 := case_when(!!!eval(event_subtypes2)),
    by = c("group_customer_no", "event_subtype")
  ]
...
contribution_stream_load <- function(...) {
  group_creditee_no <- creditee_no <- category_desc <- current_status_desc <- NULL
  ...
  contributions <- read_tessi("contributions", ...) %>%
    ...
    left_join(read_sql_table("TX_CONT_MEMB", ...),
      by = c("ref_no" = "cont_ref_no"), all.x = TRUE, suffix = c(".old", "")
    ) %>%
    left_join(read_tessi("creditees", select = c("ref_no", "creditee_no"), ...),
      by = "ref_no", all.x = TRUE, suffix = c("", ".creditee")
    ) %>%
    collect() %>%
    setDT()
  # Add creditee info
  ...
  memberships <- read_tessi("memberships", ...) %>%
    filter(!current_status_desc %in% c("Cancelled", "Deactivated")) %>%
...
#' @describeIn contribution_membership_match updates `contributions$cust_memb_no` for up to
#' one contribution per membership, intended to be called iteratively from within [contribution_membership_match]
set_contribution_membership_match <- function(contributions, memberships) {
  . <- cust_memb_no <- i.cust_memb_no <- cont_amt <- i.cont_amt <- value_amt <- memb_amt <- AVC_amt <- recog_amt <- start_amt <- score <-
    group_customer_no <- i.ref_no <- campaign_category_desc <- i.campaign_category_desc <- create_dt <- i.create_dt <- init_dt <- cont_dt <- NULL
