...
address_stream <- data.table(street="30 Lafayette Ave",
                             city="Brooklyn",
                             state="NY",
                             postal_code="11217")
...
test_that("address_geocode_census adds parsing to address_stream if it exists",{
  ...
  address_stream_parsed <- function(address_stream){cbind(address_stream,libpostal=data.table(house_number="30",road="Lafayette Ave",city="Libpostal",state="NY",postcode="11217"))}
...
test_that("address_geocode_census doesn't send duplicate addresses",{
  ...
  address_stream <- data.table(street1=rep("30 Lafayette Ave",3),
                                street2="Brooklyn Academy of Music",
                                city="Brooklyn",
                                state="NY",
                                country="USA",
                                postal_code="11217") %>% setDT
...
test_that("address_geocode_census only sends US addresses to census parser",{
  ...
  address_stream <- expand.grid(street1="30 Lafayette Ave",
              ...
              country=c("USA","UK"),
              postal_code=c("11217","1121712345","ABCDE","1","00000")) %>% lapply(as.character) %>% setDT
